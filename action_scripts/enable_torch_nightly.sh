#!/usr/bin/env bash
# Install + start the scheduled MBIRTORCH nightly (forwards to tooling/regression/enable_torch_nightly.sh).
# Platform-aware: macOS uses launchd; the cluster (Gautschi) uses a managed SLURM scrontab block
# named mbirtorch-nightly — separate from, and never touching, the mbirjax-nightly block.
...
(
  trap 'rc=$?; if [ "$rc" -ne 0 ]; then
          printf "\nenable_torch_nightly.sh failed (exit %s).\n" "$rc" >&2
          [ -t 0 ] && read -r -p "Press Enter to close... " _ </dev/tty || true
        fi' EXIT
  set -euo pipefail
  HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
  "$HERE/../tooling/regression/enable_torch_nightly.sh" "$@"
)
_rc=$?
