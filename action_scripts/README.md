...
| `enable_nightly.sh` / `disable_nightly.sh` | Start / stop the **scheduled** nightly. macOS (launchd) works now; the cluster path (scrontab + slurm) is pending the slurm script. Wrap `tooling/regression/{enable,disable}_nightly.sh`. |
