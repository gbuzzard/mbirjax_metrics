...
# Orchestrator env applied at import — BEFORE main() touches jax (toolchain_info) or spawns workers.
# claim_gpu_pool=False: run_nightly must NOT preallocate the GPU (workers get the pool via build_worker_env);
# it just seeds the log level + compile cache into os.environ so workers inherit them (see apply_env).
pt.sc.apply_env(claim_gpu_pool=False)
